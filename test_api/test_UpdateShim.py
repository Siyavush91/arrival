...
def test_change_cross_value(client, endpoint, address, duty1, freq2):
    ...
    assert response.status_code == 200
    assert isinstance(response.text, str)
    #TODO: is this Ok, if we change value in different pin in one request
...
def test_change_to_not_correct_value_(client, endpoint):
    ...
    # assert response.text == constants.MSG_INVALID_ADDRESS TODO: Internal Server Error, when enter non-existent address
